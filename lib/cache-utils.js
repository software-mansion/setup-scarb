...
async function getScarbLockfilePath() {
  ...
  const pwdOutput = await exec.getExecOutput("pwd").catch((e) => throw new Error(
      "Command `pwd` failed: " + e instanceof Error ? e.message : e
  ));

  // todo change that pwdoutput to path or some shit

  let lockfilePath;
  do {
    lockfilePath = path.join(pwdOutput.stdout, "Scarb.lock");
    // lockfilePath = pwdOutput.stdout + "/Scarb.lock";
    await fs.access(lockfilePath).catch((_) => {

    })
    const testOutput = await exec.getExecOutput("test -f " + lockfilePath).catch((_) => {
      path.join()
    });
  } while (stdout !== "/");
  ...
  // return lockfiles.reduce((prev, next) => prev.length < next.length ? prev : next);
