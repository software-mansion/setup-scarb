...
export async function determineVersion(version, { repo, nightliesRepo }) {
  ...
  if (toolVersionsPathInput != "") {
    ...
    if (!toolVersion) {
      throw new Error(
        `failed to determine version from tool-versions option, please double check the provided path`,
      );
    }
    version = toolVersion;
  }
...
async function getVersionFromToolVersionsFile(path) {
  try {
    const toolVersions = await fs.readFile(path + ".tool-versions", {
      encoding: "utf-8",
    });
