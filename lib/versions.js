...
export async function determineVersion(
  ...
  if (version != "" && toolVersionsPath != "") {
    throw new Error(
      `version and toolVersionPath cannot be BOTH provided, please either specify one or the other`,
    );
  }
...
async function getVersionFromToolVersionsFile(path_) {
  try {
    const toolVersions = await fs.readFile(path.join(path_, ".tool-versions"), {
      encoding: "utf-8",
    });
